...
class AddUserToTask @Inject constructor(
    private val repository: TasksRepository
) {
    operator fun invoke(token:String, userLogin:String, taskId:String): Flow<Resource<String>> = flow{
        try{
            emit(Resource.Loading<String>())
            val response = repository.addUserToTask(token, userLogin, taskId)
            ...
            emit(Resource.Error<String>(massage.toString()))
