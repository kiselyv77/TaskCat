...
class DeleteUserFromWorkSpace @Inject constructor(
    private val repository: TasksRepository

) {
    operator fun invoke(token: String, workSpaceId:String, userLogin: String): Flow<Resource<String>> = flow{
        try{
            emit(Resource.Loading<String>())
            val response = repository.deleteUserFromWorkSpace(token, workSpaceId, userLogin)
