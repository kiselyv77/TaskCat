...
class DeleteUserFromTask @Inject constructor(
    private val repository: TasksRepository

) {
    operator fun invoke(token: String, taskId:String, userLogin: String): Flow<Resource<String>> = flow{
        try{
            emit(Resource.Loading<String>())
            val response = repository.deleteUserFromTask(token, taskId, userLogin)
